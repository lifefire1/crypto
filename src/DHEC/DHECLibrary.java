...
public class DHECLibrary {
    public static Point getPontN(int n, Point beginPoint, Point temp, Curve curve){
    ...
    public static Point addTwoPoint(Point p, Point q, Curve curve){
        if (p.eq(q)){
    ...
    public static int findX(Point point, int m, int p){
        return superMod((m * m - 2 * point.x) % p,p);
    }


    public static int superMod(int number, int mod){
        int answer = ((number % mod) + mod) % mod;
        return answer;
    }
    public static int findY(Point point, int m, int p){
        int temp = m * ((point.x) - findX(point,m,p));
        int res = temp - point.y;
        return superMod(res,p);
    }
    public static Point nextPoint(Point point, Curve curve){
        Point res = new Point();
    ...
    public static Point nonEqPoint(Point p, Point q, Curve curve){
    ...
    private static int findNonEqY(Point p, Point q, Curve curve, int m, Point res) {
        int y = 0;
        y = p.y + m * (res.x - p.x);
        y = y * -1;
    ...
    private static int findNonEqX(Point p, Point q, Curve curve, int m) {
        int x = 0;
        x = m * m - p.x - q.x;
        return superMod(x, curve.p);
    }

    private static int findMnonEq(Point p, Point q, int pr) {
        int up = p.y - q.y;
        int down = p.x - q.x;
        if (up < 0 && down < 0){
            up = up * -1;
    ...
    public static int findM(int xP, int yP, Curve curve){
        int m = 0;
    ...
    public static int findModularInverse(int b, int mod) {
